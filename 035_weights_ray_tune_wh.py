...
class TuneReporterCallbackMetrics(TuneReporterCallback, ModelCheckpoint):
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        if (self.iteration % self.do_val == 0) | (self.iteration==1):
         ...
         # 2 should be the recon_loss (val recon_loss) >>> Metric_inserted >>> Now the metric is the 5 (to test)
            
         # OLD : ['loss', 'KL_loss', 'RecoNLL_loss', 'KL_metric', 'RecoNLL_metric']
...
#         },
        ...
        resources_per_trial={
            ...
            config={
            "intermediate_dim": tune.sample_from(lambda spec: random.choice([50])),
            "act_fun": tune.sample_from(lambda spec: random.choice(["relu"])),
            "latent_dim": tune.sample_from(lambda spec: random.choice([3])),
            "kernel_max_norm": tune.sample_from(lambda spec: random.choice([500])),
            "lr": tune.sample_from(lambda spec: random.choice([0.006])),
            "epochs": epochs,
            "weight_KL_loss": tune.sample_from(lambda spec: random.choice([0.6])),
...
if __name__ == "__main__":
    ...
    name_exp = '{}_{}_feats_scan_w'.format(dataset, len(cols_sel))
