...
def train_vae(name,name_fold, name_weights , dictionary, w, ind_w, intermediate_dim, act_fun, latent_dim, kernel_max_norm,
              lr, epochs, weight_KL_loss, batch_size,num_train=15, sig_inj = [False]):
...
#     original_dim = Nf_lognorm + Nf_PDgauss
    ...
    name_fold = 'bump_feat_{}_ld_{}'.format('_'.join([str(x) for x in selected_idx]), latent_dim)
    name_weights = 'bump_weights_{}'.format('_'.join([str(x) for x in weights if x != 0]), latent_dim)
    name = 'bump_feat_{}'.format('_'.join([str(x) for x in weights if x != 0]), latent_dim)

    train_vae(name=name,name_fold=name_fold, name_weights = name_weights, dictionary=components_dict, w=weights, ind_w = individual_weights,             intermediate_dim=50, act_fun='relu',latent_dim=latent_dim,kernel_max_norm=500, lr=0.003,epochs=2000, weight_KL_loss=0.6, 
              batch_size=200, num_train=5, sig_inj = [False])
